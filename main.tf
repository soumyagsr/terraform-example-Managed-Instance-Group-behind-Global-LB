...
resource "google_compute_instance_template" "default-mig-template" {
  provider                = "google"
...
resource "google_compute_region_instance_group_manager" "mig-mgr1" {
  ...
  version {
    name               = "ver1"
    instance_template  = "${google_compute_instance_template.default-mig-template.self_link}"
  }
  region                    = "${var.region1}"
  distribution_policy_zones = ["${var.region1_zone1}", "${var.region1_zone2}", "${var.region1_zone3}"]
  depends_on                = [google_compute_health_check.health-check, google_compute_instance_template.default-mig-template]
  named_port {
    name = "http"
    port = "80"
  }
